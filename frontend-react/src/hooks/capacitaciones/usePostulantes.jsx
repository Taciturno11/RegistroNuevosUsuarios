...
import { descargarExcel } from "../../utils/capacitaciones/excel";
...
export default function usePostulantes() {
  ...
  const loadLote = async ({ dniCap, CampañaID, mes, fechaInicio, capaNum, horariosBase }) => {
    ...
    const { postulantes, asistencias, duracion } = await api(
      `/api/capacitaciones/postulantes?campaniaID=${encodeURIComponent(CampañaID)}`+
    ...
    if (dniCap && CampañaID && mes && capaNum) {
      console.log('Llamando a /api/capacitaciones/deserciones con:', { dniCap, CampañaID, mes, capaNum });
      const desPrev = await api(
        `/api/capacitaciones/deserciones?campaniaID=${encodeURIComponent(CampañaID)}`+
    ...
    const evalPrev = await api(
      `/api/capacitaciones/evaluaciones?campania=${encodeURIComponent(CampañaID)}`+
      `&mes=${mes}&fechaInicio=${fechaInicio}`
    );
  ...
  const guardarCambios = async params => {
    ...
    try {
      if (payloadA.length) await api("/api/capacitaciones/asistencia/bulk",
        { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payloadA) });
      if (desToSend.length) await api("/api/capacitaciones/deserciones/bulk",
        { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(desToSend) });
      if (payloadE.length) await api("/api/capacitaciones/evaluaciones/bulk",
        { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payloadE) });
      if (payloadEstados.length) await api("/api/capacitaciones/postulantes/estado",
        { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payloadEstados) });
      if (payloadPostulantes.length) await api("/api/capacitaciones/postulantes/horario",
