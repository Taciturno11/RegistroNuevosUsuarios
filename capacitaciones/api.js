...
router.get("/capacitadores/:dni", async (req, res) => {
  ...
  try {
    const { recordset } = await R().input("dni", sql.VarChar(20), dni).query(`
...
router.get("/postulantes", authMiddleware, async (req, res) => {
  ...
  try {
    const post = await R()
      ...
      .input("fechaIni", sql.VarChar(10), fechaInicio).query(`
    ...
    if (asis.recordset.length === 0 && asisFallback.recordset.length > 0) {
      console.log(
        `[API] Consulta principal sin resultados, usando fallback por mes para ${dniCap}`
      );
      asistenciasFinales = asisFallback.recordset;
    } else if (
      asis.recordset.length > 0 &&
      asisFallback.recordset.length > asis.recordset.length
    ) {
      // Si el fallback tiene más datos, combinar ambos (eliminando duplicados)
      console.log(
        `[API] Combinando consultas para obtener máximo de asistencias para ${dniCap}`
      );
    ...
    console.log(
      `[API] Asistencias finales para ${dniCap}: ${asistenciasFinales.length} registros`
    );
...
/* ────────────── ENDPOINT PARA DASHBOARD JEFA ──────────────── */
// Obtener detalle de postulantes de una capacitación específica (SIN autenticación)
router.get("/capacitacion/detalle", async (req, res) => {
  try {
    const { campaniaID, fechaInicio, dniCapacitador } = req.query;

    console.log("📥 GET /capacitacion/detalle - Parámetros recibidos:", {
      campaniaID,
      fechaInicio,
      dniCapacitador,
    });
    ...
    if (!campaniaID || !fechaInicio || !dniCapacitador) {
      console.error("❌ Faltan parámetros requeridos");
      return res.status(400).json({
        success: false,
        message:
          "Parámetros requeridos: campaniaID, fechaInicio, dniCapacitador",
      });
    }

    // Extraer el mes de la fecha de inicio (formato: YYYY-MM)
    const mes = fechaInicio.substring(0, 7);

    console.log("🔍 Buscando postulantes con:", {
      dniCapacitador,
      campaniaID,
      mes,
      fechaInicio,
    ...
    const postulantes = await R()
      .input("dniCap", sql.VarChar(20), dniCapacitador)
      .input("camp", sql.VarChar(50), String(campaniaID).split(",")[0].trim())
      .input("prefijo", sql.VarChar(7), mes)
      .input("fechaIni", sql.VarChar(10), fechaInicio).query(`
        SELECT pf.DNI AS dni,
               CONCAT(pf.Nombres,' ',pf.ApellidoPaterno,' ',pf.ApellidoMaterno) AS nombre,
        ...
        WHERE pf.DNI_Capacitador = @dniCap
          AND (
            CAST(pf.CampañaID AS VARCHAR) = @camp
            OR c.NombreCampaña = @camp
          )
          AND FORMAT(pf.FechaInicio,'yyyy-MM') = @prefijo
          AND FORMAT(pf.FechaInicio,'yyyy-MM-dd') = @fechaIni
        ORDER BY pf.Nombres, pf.ApellidoPaterno
      `);

    console.log(`✅ Postulantes encontrados: ${postulantes.recordset.length}`);

    if (postulantes.recordset.length === 0) {
      console.log(
        "⚠️ No se encontraron postulantes con los parámetros proporcionados"
      );
    } else {
      // 🔍 DEBUG: Mostrar estados de todos los postulantes
      console.log('📊 Estados de postulantes encontrados:');
      const estadosCounts = {};
      postulantes.recordset.forEach(p => {
        const estado = p.EstadoPostulante || 'Sin Estado';
        estadosCounts[estado] = (estadosCounts[estado] || 0) + 1;
      });
      console.log(estadosCounts);
      console.log('📋 Primeros 5 postulantes:', postulantes.recordset.slice(0, 5).map(p => ({
        nombre: p.Nombres + ' ' + p.ApellidoPaterno,
        estado: p.EstadoPostulante,
        fechaCese: p.FechaCese
      })));
    }

    // Mapear postulantes al formato esperado por el frontend
    const postulantesFormateados = postulantes.recordset.map((p) => ({
      dni: p.dni,
      apellidoPaterno: p.ApellidoPaterno || "",
      nombres: p.Nombres || p.nombre.split(" ")[0] || p.nombre,
      estado: p.EstadoPostulante || "En Proceso",
      fechaCese: p.FechaCese || null,
    }));

    console.log(
      "📤 Enviando respuesta con",
      postulantesFormateados.length,
      "postulantes"
    );

    res.json({
      success: true,
      data: postulantesFormateados,
      total: postulantesFormateados.length,
    });
  } catch (error) {
    console.error("❌ Error en /capacitacion/detalle:", error);
    console.error("Stack:", error.stack);
    res.status(500).json({
      success: false,
      message: "Error al obtener detalle de capacitación",
      error: error.message,
    });
  }
});
...
router.post("/deserciones/bulk", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const { recordset: desercionesActuales } = await tx.request().query(`
    ...
    console.log(
      "Deserciones actuales encontradas:",
      desercionesActuales.length
    );
    console.log(
      "No se eliminarán deserciones automáticamente para permitir múltiples deserciones"
    );
    ...
    for (const r of req.body) {
      ...
      await tx
        ...
        .input("fechaInicio", sql.Date, r.fecha_inicio).query(`
      ...
      await tx
        ...
        .input("dni", sql.VarChar(20), r.postulante_dni)
        .input("fechaDes", sql.Date, r.fecha_desercion)
        .input("capa", sql.Int, r.capa_numero).query(`
    ...
    res
      .status(500)
      .json({ error: "No se pudo guardar deserciones", details: e.message });
  }
});
...
router.get("/evaluaciones", authMiddleware, async (req, res) => {
  ...
  console.log("[API] GET /evaluaciones - Parámetros recibidos:", {
    dniCap,
    campania,
    mes,
    fechaInicio,
  ...
  try {
    const { recordset } = await R()
      ...
      .input("prefijo", sql.VarChar(7), mes)
      .input("fechaInicio", sql.VarChar(10), fechaInicio).query(`
      ...
      `);
    console.log(
      "[API] GET /evaluaciones - Resultados encontrados:",
      recordset.length,
      "evaluaciones"
    );
    console.log(
      "[API] GET /evaluaciones - Primeras 3 evaluaciones:",
      recordset.slice(0, 3)
    );
    res.json(recordset);
  } catch (e) {
  ...
  try {
    for (const ev of req.body) {
      ...
      await tx
        ...
        .input("nota", sql.Decimal(4, 1), ev.nota).query(`
...
WHEN NOT MATCHED THEN
  ...
  VALUES (@dni,@fecha,@nota);
    ...
    res
      .status(500)
      .json({
        error: "No se pudieron guardar evaluaciones",
        details: e.message,
      });
  }
});
  ...
  try {
    for (const r of req.body) {
      ...
      if (r.estado_asistencia !== "D") {
        ...
        if (!campaniaID) {
          console.log(
            `CampañaID es undefined, buscando en Postulantes_En_Formacion...`
          );
          const campRes = await tx
            .request()
            .input("dni", sql.VarChar(20), r.postulante_dni)
            .input("fechaInicio", sql.Date, r.fecha_inicio).query(`
        ...
        if (campaniaID) {
          ...
          const checkDeserciones = await tx
            .request()
            .input("dni", sql.VarChar(20), r.postulante_dni)
          ...
          const checkTodasDeserciones = await tx
            .request()
            .input("dni", sql.VarChar(20), r.postulante_dni).query(`
              SELECT * FROM Deserciones_Formacion 
              WHERE postulante_dni = @dni
            `);
          ...
          console.log(
            `No se pudo encontrar CampañaID para DNI: ${r.postulante_dni}`
          );
        }
      }
      ...
      const asistenciaResult = await tx
        .request()
        .input("dni", sql.VarChar(20), r.postulante_dni)
        ...
        .input("CampañaID", sql.Int, r.CampañaID).query(`
...
WHEN NOT MATCHED THEN
  ...
  VALUES (@dni,@fecha,@etapa,@estado,@capa,@fechaInicio,@CampañaID);
    ...
    res
      .status(500)
      .json({ error: "No se pudo guardar asistencias", details: e.message });
  }
});
...
router.get("/meses", authMiddleware, async (req, res) => {
  const dniCap = req.user.dni;
  try {
    const { recordset } = await R().input("dniCap", sql.VarChar(20), dniCap)
      .query(`
...
router.get("/dashboard-coordinadora/:dni", async (req, res) => {
  ...
  try {
    ...
    const capacitadoresResult = await R().input(
      "jefeDni",
      sql.VarChar(20),
      dniCoordinadora
    ).query(`
    ...
    if (capa && campania) {
      const capaResult = await R()
        ...
        .input("capa", sql.Int, parseInt(capa)).query(`
          SELECT FechaInicio
          FROM (
            SELECT 
              pf.FechaInicio,
    ...
    if (fechaInicioCapa)
      queryPostulantes += ` AND FechaInicio = @fechaInicioCapa`;
    ...
    if (fechaInicioCapa)
      queryDeserciones += ` AND p.FechaInicio = @fechaInicioCapa`;
    ...
    if (mes)
      queryDesercionesATH1 += ` AND FORMAT(p.FechaInicio,'yyyy-MM') = @mes`;
    if (fechaInicioCapa)
      queryDesercionesATH1 += ` AND p.FechaInicio = @fechaInicioCapa`;
    ...
    if (mes)
      queryDesercionesATH2 += ` AND FORMAT(p.FechaInicio,'yyyy-MM') = @mes`;
    if (fechaInicioCapa)
      queryDesercionesATH2 += ` AND p.FechaInicio = @fechaInicioCapa`;
    ...
    `;
    if (campania)
      queryDesercionesATHFormacion += ` AND p.CampañaID = @campania`;
    if (mes)
      queryDesercionesATHFormacion += ` AND FORMAT(p.FechaInicio,'yyyy-MM') = @mes`;
    if (fechaInicioCapa)
      queryDesercionesATHFormacion += ` AND p.FechaInicio = @fechaInicioCapa`;
    ...
    `;
    if (campania)
      queryDesercionesSinCategoria += ` AND p.CampañaID = @campania`;
    if (mes)
      queryDesercionesSinCategoria += ` AND FORMAT(p.FechaInicio,'yyyy-MM') = @mes`;
    if (fechaInicioCapa)
      queryDesercionesSinCategoria += ` AND p.FechaInicio = @fechaInicioCapa`;
    ...
    if (mes)
      queryPostulantesDia2 += ` AND FORMAT(p.FechaInicio,'yyyy-MM') = @mes`;
    if (fechaInicioCapa)
      queryPostulantesDia2 += ` AND p.FechaInicio = @fechaInicioCapa`;
    ...
    const totalContratados = postulantes.filter(
      (p) => p.EstadoPostulante === "Contratado"
    ).length;
    const totalPostulantesDia2 = postulantesDia2.length;
    const porcentajeExito =
      totalPostulantesDia2 > 0
        ? Math.round((totalContratados / totalPostulantesDia2) * 100)
        : 0;

    // Calcular porcentajes de deserciones
    const porcentajeDesercionesATH1 =
      totalPostulantes > 0
        ? Math.round((totalDesercionesATH1 / totalPostulantes) * 100)
        : 0;
    const porcentajeDesercionesATH2 =
      totalPostulantes > 0
        ? Math.round((totalDesercionesATH2 / totalPostulantes) * 100)
        : 0;
    const porcentajeDesercionesATHFormacion =
      totalPostulantes > 0
        ? Math.round((totalDesercionesATHFormacion / totalPostulantes) * 100)
        : 0;

    // Calcular deserciones totales como suma de las específicas
    const totalDesercionesCalculado =
      totalDesercionesATH1 +
      totalDesercionesATH2 +
      totalDesercionesATHFormacion;
    ...
    console.log(
      "Cálculo:",
      `${totalContratados} / ${totalPostulantesDia2} × 100 = ${porcentajeExito}%`
    );
    console.log(
      "Postulantes día 2:",
      postulantesDia2.map((p) => p.DNI)
    );
    console.log("=====================");
    ...
    console.log(
      "DNIs ATH1:",
      desercionesATH1.map((d) => d.postulante_dni)
    );
    console.log(
      "DNIs ATH2:",
      desercionesATH2.map((d) => d.postulante_dni)
    );
    console.log(
      "DNIs Formación:",
      desercionesATHFormacion.map((d) => d.postulante_dni)
    );

    // Verificar deserciones totales
    console.log(
      "DNIs Deserciones totales:",
      deserciones.map((d) => d.postulante_dni)
    );

    // Verificar deserciones sin categoría
    console.log(
      "Deserciones sin categoría (DATEDIFF < 0):",
      desercionesSinCategoria.length
    );
    console.log(
      "Detalles deserciones sin categoría:",
      desercionesSinCategoria.map((d) => ({
    ...
    const tablaCapacitadores = capacitadores.map((cap) => {
      ...
      const desercionesCount = deserciones.filter(
        (d) => d.DNI_Capacitador === cap.DNI
      ).length;
      const contratadosCount = posts.filter(
        (p) => p.EstadoPostulante === "Contratado"
      ).length;
      const porcentaje =
        postulantesCount > 0
          ? Math.round((contratadosCount / postulantesCount) * 100)
          : 0;

      // Deserciones específicas por capacitador
      const desercionesATH1Count = desercionesATH1.filter(
        (d) => d.DNI_Capacitador === cap.DNI
      ).length;
      const desercionesATH2Count = desercionesATH2.filter(
        (d) => d.DNI_Capacitador === cap.DNI
      ).length;
      const desercionesATHFormacionCount = desercionesATHFormacion.filter(
        (d) => d.DNI_Capacitador === cap.DNI
      ).length;
    ...
    res
      .status(500)
      .json({ error: "Error al obtener dashboard", details: e.message });
  }
});
...
router.post("/login", async (req, res) => {
  ...
  try {
    const result = await R().input("dni", sql.VarChar(20), usuario).query(`
...
router.post("/postulantes/estado", async (req, res) => {
  ...
  try {
    for (const p of req.body) {
      ...
      if (p.estado === "Desaprobado" && p.fechaCese) {
        await tx
          ...
          .input("fechaCese", sql.Date, p.fechaCese).query(`
            UPDATE Postulantes_En_Formacion
        ...
        await tx
          ...
          .input("CampañaID", sql.Int, Number(p.CampañaID))
          .input("fechaInicio", sql.Date, p.fecha_inicio)
          .input("estado", sql.VarChar(20), p.estado).query(`
            UPDATE Postulantes_En_Formacion
    ...
    res
      .status(500)
      .json({
        error: "No se pudo actualizar el estado final",
        details: e.message,
      });
  }
});
  ...
  try {
    for (const p of lista) {
      ...
      const result = await tx
        ...
        .query(
          "SELECT GrupoID FROM GruposDeHorario WHERE NombreGrupo = @nombreGrupo"
        );
      ...
      await tx
        .request()
        .input("dni", sql.VarChar(20), p.dni)
        .input("grupoID", sql.Int, grupo.GrupoID).query(`
          UPDATE Postulantes_En_Formacion
    ...
    res
      .status(500)
      .json({
        error: "Error al actualizar GrupoHorarioID",
        details: e.message,
      });
  }
});
...
router.get("/horarios-base", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error al obtener horarios base", details: e.message });
  }
});
  ...
  async (req, res) => {
    try {
      ...
      await R()
        ...
        .input("fotoPerfil", sql.VarChar(255), photoUrl).query(`
...
router.get("/photo/:dni", async (req, res) => {
  try {
    const { dni } = req.params;
    const result = await R().input("dni", sql.VarChar(20), dni).query(`
        SELECT FotoPerfil 
        FROM PRI.Empleados 
        WHERE DNI = @dni
      `);
...
router.delete("/photo", authMiddleware, async (req, res) => {
  try {
    ...
    await R().input("dni", sql.VarChar(20), dni).query(`
        UPDATE PRI.Empleados 
        SET FotoPerfil = NULL 
        WHERE DNI = @dni
      `);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      await R()
        .input("dni", sql.VarChar(20), dni)
        .input("fotoUrl", sql.VarChar(500), fotoUrl).query(`
...
router.delete("/fotos-perfil/:dni", authMiddleware, async (req, res) => {
  try {
    ...
    if (dni !== req.params.dni) {
      return res
        .status(403)
        .json({ error: "No autorizado para eliminar esta foto" });
    }
...
router.get("/capacitaciones/resumen-jefe", async (req, res) => {
  ...
  try {
    ...
    const qEntreRows = await R().query(
      `SELECT CampañaID, FechaInicio, DNI_Capacitador, qEntre FROM QEntre_Jefe`
    );
    const qEntreMap = {};
    qEntreRows.recordset.forEach((q) => {
      qEntreMap[
        `${q.CampañaID}_${q.FechaInicio.toISOString().slice(0, 10)}_${
          q.DNI_Capacitador
        }`
      ] = q.qEntre;
    });
    ...
    postRows.recordset.forEach((p) => {
      listaMap[
        `${p.CampañaID}_${p.FechaInicio.toISOString().slice(0, 10)}_${
          p.DNI_Capacitador
        }`
      ] = p.lista;
    });
    console.log(
      "DEBUG: Lista de postulantes encontrados:",
      postRows.recordset.length
    );
    console.log(
      "DEBUG: Primeros 3 registros de lista:",
      postRows.recordset.slice(0, 3)
    );

    // 3.5. Traer conteo de estados por lote (para detectar campañas sin evaluación)
    const estadosRows = await R().query(`
      SELECT CampañaID, FechaInicio, DNI_Capacitador, EstadoPostulante, COUNT(*) AS cantidad
      FROM Postulantes_En_Formacion
      WHERE FechaInicio IS NOT NULL
      GROUP BY CampañaID, FechaInicio, DNI_Capacitador, EstadoPostulante
    `);
    const estadosMap = {};
    estadosRows.recordset.forEach((e) => {
      const key = `${e.CampañaID}_${e.FechaInicio.toISOString().slice(0, 10)}_${
        e.DNI_Capacitador
      }`;
      if (!estadosMap[key]) {
        estadosMap[key] = { Contratado: 0, Desaprobado: 0, Capacitacion: 0, Deserto: 0 };
      }
      const estado = e.EstadoPostulante || 'Capacitacion';
      if (estado === 'Contratado') estadosMap[key].Contratado = e.cantidad;
      else if (estado === 'Desaprobado') estadosMap[key].Desaprobado = e.cantidad;
      else if (estado === 'Desertó') estadosMap[key].Deserto = e.cantidad;
      else estadosMap[key].Capacitacion = e.cantidad;
    });
    console.log(
      "DEBUG: Estados de postulantes encontrados:",
      estadosRows.recordset.length
    );
    ...
    `);
    console.log(
      "DEBUG: Total asistencias encontradas:",
      asisRows.recordset.length
    );
    console.log(
      "DEBUG: Primeras 5 asistencias:",
      asisRows.recordset.slice(0, 5)
    );

    // Debug específico para Josep (75707924)
    const asistenciasJosep = asisRows.recordset.filter(
      (a) => a.DNI_Capacitador === "75707924"
    );
    console.log(
      "DEBUG JOSEP: Total asistencias para Josep:",
      asistenciasJosep.length
    );
    ...
    const asistenciasJosepHogar = asistenciasJosep.filter((a) => {
      const key = `${a.CampañaID}_${a.FechaInicio.toISOString().slice(0, 10)}_${
        a.DNI_Capacitador
      }`;
      return key.includes("2025-07-02"); // Fecha específica del problema
    });
    console.log(
      "DEBUG JOSEP HOGAR: Asistencias para Hogar 2025-07-02:",
      asistenciasJosepHogar
    );
    const asisMap = {};
    asisRows.recordset.forEach((a) => {
      ...
      asisMap[key][a.fecha.toISOString().slice(0, 10)].push(
        a.estado_asistencia
      );
    });
    ...
    desercionesFiltradas.forEach((d) => {
      const key = `${d.CampañaID}_${d.FechaInicio.toISOString().slice(0, 10)}_${
        d.DNI_Capacitador
      }`;
    ...
    console.log(
      "DEBUG: Total deserciones encontradas:",
      todasDeserciones.recordset.length
    );
    console.log(
      "DEBUG: Deserciones filtradas (día 3+):",
      desercionesFiltradas.length
    );
    ...
    let allRows = rows.map((lote) => {
      ...
      const key = `${campaniaId}_${lote.FechaInicio.toISOString().slice(
        0,
        10
      )}_${lote.DNI_Capacitador}`;
      ...
      if (lote.DNI_Capacitador === "75707924") {
        console.log(
          `DEBUG JOSEP - Procesando lote: ${
            lote.NombreCampaña
          } - ${lote.FechaInicio.toISOString().slice(0, 10)}`
        );
        console.log(`DEBUG JOSEP - Key generada: ${key}`);
        console.log(`DEBUG JOSEP - ¿Existe en asisMap?`, !!asisMap[key]);
        if (asisMap[key]) {
          console.log(
            `DEBUG JOSEP - Fechas disponibles:`,
            Object.keys(asisMap[key])
          );
        }
      }
      ...
      if (asisMap[key] && asisMap[key][primerDiaFecha]) {
        ...
        primerDia = asisMap[key][primerDiaFecha].filter(
          (estado) => estado === "A"
        ).length;
        console.log(
          `DEBUG ${lote.NombreCampaña} - Primer día: ${primerDiaFecha}, Asistencias totales: ${asisMap[key][primerDiaFecha].length}, Asistencias 'A': ${primerDia}`
        );
      }
      ...
      if (lote.DNI_Capacitador === "75707924") {
        ...
        console.log(
          `DEBUG JOSEP - ¿Tiene asistencias en primer día?`,
          !!(asisMap[key] && asisMap[key][primerDiaFecha])
        );
        if (asisMap[key] && asisMap[key][primerDiaFecha]) {
          console.log(
            `DEBUG JOSEP - Asistencias primer día:`,
            asisMap[key][primerDiaFecha]
          );
        }
      }
      // % EFEC ATH
      const porcentajeEfec =
        esperado > 0 ? Math.round((primerDia / esperado) * 100) : 0;
      ...
      if (asisMap[key]) {
        ...
        for (let dia = 0; dia < 31; dia++) {
          ...
          if (asisMap[key][fechaStr]) {
            ...
            asistencias[dia] = asisMap[key][fechaStr].filter(
              (estado) => estado === "A"
            ).length;
          }
        }
        ...
        console.log(
          `DEBUG ${lote.NombreCampaña} - Fechas con asistencias:`,
          Object.keys(asisMap[key] || {})
        );
        console.log(
          `DEBUG ${lote.NombreCampaña} - Primeros 7 días de asistencias:`,
          asistencias.slice(0, 7)
        );

        // Debug específico para Josep - asistencias
        if (lote.DNI_Capacitador === "75707924") {
          console.log(
            `DEBUG JOSEP - Asistencias calculadas:`,
            asistencias.slice(0, 7)
          );
          console.log(
            `DEBUG JOSEP - Fechas consecutivas:`,
            fechasConsecutivas.slice(0, 7)
          );
        }
      }
      // Activos = lista - bajas
      ...
      if (asisMap[key] && asisMap[key][dia2Fecha]) {
        ...
        postulantesDia2 = asisMap[key][dia2Fecha].filter(
          (estado) =>
      ...
      const porcentajeDeser =
        postulantesDia2 > 0 ? Math.round((qBajas / postulantesDia2) * 100) : 0;

      // Debug: Log para verificar los valores
      console.log(
        `DEBUG ${lote.NombreCampaña}: lista=${lista}, qBajas=${qBajas}, activos=${activos}, primerDia=${primerDia}, porcentajeDeser=${porcentajeDeser}%`
      );
      console.log(
        `DEBUG ${lote.NombreCampaña}: Nueva fórmula - dia2Fecha=${dia2Fecha}, postulantesDia2=${postulantesDia2}, qBajas=${qBajas}, porcentajeDeser=${porcentajeDeser}%`
      );
      ...
      // Obtener estados de postulantes para esta campaña
      const estados = estadosMap[key] || { Contratado: 0, Desaprobado: 0, Capacitacion: 0, Deserto: 0 };
      const necesitaEvaluacion = finalizado && estados.Contratado === 0 && estados.Desaprobado === 0;
      ...
      return {
        ...
        estados, // ⚠️ NUEVO: Conteos de estados
        necesitaEvaluacion, // ⚠️ NUEVO: Flag para detectar si falta evaluación
      };
    });
    ...
    res
      .status(500)
      .json({ error: "Error al obtener resumen de capacitaciones" });
  }
});
...
router.post("/qentre-jefe", async (req, res) => {
  ...
  console.log("POST /qentre-jefe - Datos extraídos:", {
    CampañaID,
    FechaInicio,
    DNI_Capacitador,
    qEntre,
  ...
  if (!CampañaID || !FechaInicio || !DNI_Capacitador || qEntre == null) {
    ...
    return res
      .status(400)
      .json({ error: "Faltan datos requeridos", received: req.body });
  }
  try {
    await R()
      ...
      .input("qEntre", sql.Int, qEntre).query(`
...
router.get("/dashboard-coordinadora/:dni/capas", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Error al obtener capas", details: e.message });
  }
});
